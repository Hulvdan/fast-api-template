...
def get_app_object_access(
    session: Session = Depends(get_database), access: TokenAccess = Depends()
):
    return get_object_or_404(
        session.query(App).filter(
            App.client_secret == access.client_secret, App.client_id == access.client_id
        )
    )
